...
def test_config():
    ...
    assert config.llm.deepseek.api_key and "YOUR" not in config.llm.deepseek.api_key, (
        "DeepSeek API Key 未配置"
    )
    assert config.video_gen.kling.api_key and "YOUR" not in config.video_gen.kling.api_key, (
        "Kling API Key 未配置"
    )
    assert config.tts.api_key and "YOUR" not in config.tts.api_key, "TTS API Key 未配置"
    assert config.image_gen.api_key and "YOUR" not in config.image_gen.api_key, (
        "图像生成 API Key 未配置"
    )
    print(
        f"  LLM: {config.llm.default_provider} | Video: {config.video_gen.default_provider} | TTS: {config.tts.default_provider}"
    )
...
def test_llm_script():
    ...
    config = load_config("configs/config.yaml")
    script = asyncio.run(
        generate_script(topic="一只猫咪在阳光下打盹，温馨治愈，15秒", config=config, verbose=True)
    )
    ...
    for s in script.scenes:
        ...
        assert s.shot_mode in ["i2v", "multi_ref", "first_end_frame", "t2v"], (
            f"shot_mode 无效: {s.shot_mode}"
        )
...
def test_tts():
    ...
    result_path, duration = asyncio.run(
        generate_voiceover(scene=scene, output_dir="/tmp/tts_test", config=config, verbose=True)
    )
...
def test_image_gen():
    ...
    result_path = asyncio.run(
        generate_keyframe(scene=scene, output_dir=output_dir, config=config, verbose=True)
    )
...
def test_kling_api():
    ...
    print(
        f"  注意: Kling API 实际调用已跳过（避免消耗配额），最终交付时会单独进行一次完整连通性测试"
    )
